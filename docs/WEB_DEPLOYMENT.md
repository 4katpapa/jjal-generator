...
DNS는 내도메인.한국에서 관리합니다. 웹포워딩·단일 HTML 페이지와 네임서버 이전은 사용하지 않습니다. 2026-09-13 관리 화면에서 확인한 도메인 만료일은 **2026-12-13**이며, **만료 30일 전인 2026-11-13부터 연장**할 수 있습니다. 이것은 무료 도메인의 이용 기간이며 SSL 인증서의 만료일과는 다릅니다. 연장 후에는 관리 화면의 새 만료일을 확인합니다.
...
2026-09-13 새 주소의 Search Console HTML 태그 소유권 확인을 완료했습니다. `https://jjaltool.kro.kr/sitemap.xml`은 **성공**으로 처리되었으며 발견된 페이지는 1개입니다. 홈페이지의 **색인 생성 요청**도 접수되어 우선순위 크롤링 대기열에 추가되었습니다. 요청 당시 새 주소는 아직 색인되지 않았으며, 요청 접수는 색인 완료나 검색 노출을 의미하지 않습니다. 기존 주소에서 확인한 색인 상태와 새 주소의 색인 상태는 별개입니다.
